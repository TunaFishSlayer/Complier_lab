...
void compileBlock(void) {
  ...
  if (lookAhead->tokenType == KW_CONST) {
    ...
    do {
      eat(TK_IDENT);
      cnst = createConstantObject(currentToken->string);
      eat(SB_EQ);
      cnst->constAttrs->value = compileConstant();
      eat(SB_SEMICOLON);
      declareObject(cnst);
...
void compileBlock3(void) {
  ...
  if (lookAhead->tokenType == KW_VAR) {
    ...
    do {
      eat(TK_IDENT);
      var = createVariableObject(currentToken->string);
      eat(SB_COLON);
...
void compileFuncDecl(void) {
  ...
  eat(KW_FUNCTION);
  eat(TK_IDENT);
  func = createFunctionObject(currentToken->string);
